// Problem: F - Balanced Rectangles
// Contest: AtCoder - AtCoder Beginner Contest 410
// URL: https://atcoder.jp/contests/abc410/tasks/abc410_f
...
void solve(bool SPE){ 
	...
	foru(i,1,n){
		foru(j,1,m){
			pre[i][j]=pre[i-1][j]+pre[i][j-1]-pre[i-1][j-1]+(s[i][j]=='.'?1:-1);
		}
	}
	...
	foru(l,1,n){
		foru(r,l,n){
			mp.clear();
			mp[pre[r][0]-pre[l-1][0]]=1;
			foru(i,1,m){
				ans+=mp[pre[r][i]-pre[l-1][i]];
				// cout<<l<<' '<<r<<' '<<i<<' '<<mp[pre[r][i]-pre[l-1][i]]<<endl;
				mp[pre[r][i]-pre[l-1][i]]++;
			}
		}
	}
