...
// Contest: AtCoder - AtCoder Beginner Contest 410
...
void modify(int p,int pos,int k){
	...
	push_up(p);
	// cout<<p<<' '<<tr[p].l<<' '<<tr[p].r<<' '<<k<<' '<<tr[p].mx<<endl;
}
...
void solve(bool SPE){ 
	...
	sort(a+1,a+n+1,[](pair<int,int> x,pair<int,int> y)->bool{
		return x.se<y.se;
	});
	...
	sort(b+1,b+n+1,[](pair<int,int> x,pair<int,int> y)->bool{
		return x.fi<y.fi;
	});
	...
	g[n+1]=0;
	for(int i=n;i>=1;i--){
		g[i]=1+query(1,1,b[i].se-1);
		// for(int j=i+1;j<=n;j++){
			// if(b[j].se<b[i].se){
				// chkmax(g[i],g[j]+1);
			// }
		// }
		// cout<<g[i]<<' '<<b[i].se<<endl;
		modify(1,b[i].se,g[i]);
		// cout<<query(1,1,b[i].se)<<endl;
	}
	...
	int j=0,k=1;
	foru(i,1,2*n){
		while(j+1<=n && a[j+1].se<i)	j++;
		while(k<=n && b[k].fi<i)	k++;
		chkmax(ans,f[j]+g[k]);
	}
