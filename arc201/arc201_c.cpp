// Problem: C - Prefix Covering
// Contest: AtCoder - AtCoder Regular Contest 201
...
void push_up(int p){
	...
	if(tr[p].hv){
		mdd(f[p][0],mul(f[lc][0],f[rc][0]));
		mdd(f[p][0],mul(f[lc][0],f[rc][1]));
		mdd(f[p][0],mul(f[lc][1],f[rc][0]));
		mdd(f[p][1],mul(f[lc][1],f[rc][1]));
		
		mdd(f[p][1],mul(f[lc][0],f[rc][0]));
		mdd(f[p][1],mul(f[lc][0],f[rc][1]));
		mdd(f[p][1],mul(f[lc][1],f[rc][0]));
		mdd(f[p][1],mul(f[lc][1],f[rc][1]));
...
void insert(int& p,string& s,int i){
	if(!p){
		p=++pcnt;
	}
	if(i==sz(s)){
	...
	if(s[i]=='A'){
		insert(tr[p].lc,s,i+1);
	}else{
		insert(tr[p].rc,s,i+1);
	}
	push_up(p);
}
