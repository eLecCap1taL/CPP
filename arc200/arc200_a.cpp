// Problem: A - Dot Product
// Contest: AtCoder - AtCoder Regular Contest 200 (Div. 2)
// URL: https://atcoder.jp/contests/arc200/tasks/arc200_a
...
void solve(bool SPE){ 
	...
	pair<LL,LL> L=a[1],R=a[1];
	
	foru(i,2,n){
		if(a[i].se*L.fi<L.se*a[i].fi)	L=a[i];
		if(a[i].se*R.fi>R.se*a[i].fi)	R=a[i];
	}
	
	if(L.se*R.fi==L.fi*R.se){
		cout<<"No\n";
		return ;
	}
	cout<<"Yes\n";
	...
	// LL X=C1*L.fi+C2*R.fi;
	// LL Y=C1*L.se+C2*R.se;
