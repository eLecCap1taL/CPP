...
pair<int,int> icomp(int x){
	return mkp(x>>14,x&((1<<14)-1));
}
...
void solve(bool SPE){ 
	...
	foru(i,0,n-1){
		...
		for(const auto [sta,v]:g[i&1]){
			...
			//choose
			if(i+1>=t[j+1]){
				mdd(g[(i+1)&1][comp(j+1,max(k,j+1))],v);
			}
			//not choose
			int w=i-j+1;
			mdd(g[(i+1)&1][comp(j,max(k,p[i+1]-w))],v);
		}
	}
	exit(0);
