...
template<class ... T>
int add(const int x,const T ... xs){
	int ret=x+add(xs...);
	if(ret>=mod)	ret-=mod;
	return ret;
}
...
template<class ... T>
int mul(const int x,const T ... xs){
	return (LL)x*(LL)mul(xs...)%mod;
}
	...
	void work(){
		...
		foru(o,1,q){
			...
			if(ty==1){
				...
				ans+=s%mod*(LL)(r-l+1-l*r)*2ll%mod;
