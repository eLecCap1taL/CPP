...
int calc(int K){
	...
	foru(i,1,m){
		...
		foru(j,0,N){
			...
			int r=j;
			mll(C,inv[n+m-i+1]);
			f[i][j]=mul(C,rmv(h[r],l==0?0:h[l-1]));
			// foru(o,l,r){
			// 	mdd(f[i][j],mul(h[o],C));
			// }
		}
	...
	static int g[105][2005];
	memset(g,0,sizeof g);
	// auto split=[](int n,int m)->int{
	// 	//n个球，分出m组，组不同，允许空
	// 	n+=m;
	// 	//n个球，分出m组，组不同，不允许空
	// 	// cerr<<n-1<<' '<<m-1<<' '<<C[n-1][m-1]<<endl;
	// 	return C[n-1][m-1];
	// };
	...
	foru(i,1,n){
		foru(j,0,N){
			foru(o,0,min(j,c[i])){
				mdd(g[i][j],mul(g[i-1][j-o],C[j][j-o]));
			}
		}
	}

	// cerr<<mul(f[1][0][1],4)<<endl;
	// cerr<<mul(f[4][2][0],16,9)<<endl;
	foru(i,0,N){
		//use f[i][m] to calc
		// cerr<<f[i][m][0]<<' '<<g[n][i-m]<<endl;
		// cerr<<f[m][i]<<' '<<g[n][i]<<endl;
		mdd(ans,mul(f[m][i],g[n][i]));	
	}
...
signed main()
{
	...
	#ifndef CPEDITOR
	if(freopen("water2.in","r",stdin));
	// if(freopen("water.out","w",stdout));
	#ifdef ONLINE_JUDGE
	if(freopen("water.in","r",stdin));
	if(freopen("water.out","w",stdout));
	#endif
	#endif
