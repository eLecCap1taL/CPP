...
void solve(bool SPE){ 
	...
	for(int s=1;s<(1<<n);s++){
		...
		for(int j=0;j<n;j++){
			for(int k=lb;k<n;k++){
				if(k==lb){
					mdd(ans,mul(f[s][j],e[j][lb]));
				}
				if((s>>k)&1)	continue;
				mdd(f[s|(1<<k)][k],mul(f[s][j],e[j][k]));
			}
		}
	}
	...
	foru(i,0,n-1){
		foru(j,i,n-1){
			// cout<<e[i][j]*(e[i][j]-1)<<endl;
			mdd(ans,mul(_2,e[i][j],rmv(e[i][j],1)));
		}
	}
