...
int handle_single(bitset<1005> b){
	int ans=0;
	while(1){
		int h=-1;
		for(int j=b._Find_first();j<=m;j=b._Find_next(j)){
			h=j;
		}
...
int handle_fast(bitset<1005> b){
	int mx=handle_single(b);
...
namespace SUBN2{
	void work(){
		int ans[3]={0,handle_single(a[1]),handle_single(a[2])};
		ans[0]=max(ans[1],ans[2])+1;

		chkmin(ans[0],max(ans[2]+handle_fast(a[1]),ans[1]));
		chkmin(ans[0],max(ans[1]+handle_fast(a[2]),ans[2]));
...
signed main()
{
	...
	#ifndef CPEDITOR
	#ifndef ONLINE_JUDGE
	// if(freopen("pills1.in","r",stdin));
	if(freopen("pills.in","r",stdin));
	if(freopen("pills.out","w",stdout));
	#endif
	#endif
