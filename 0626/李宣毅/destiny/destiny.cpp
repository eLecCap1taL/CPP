...
void solve(bool SPE){ 
	...
	foru(i,1,m){
		...
		if(op==0 && i<m)	continue;

		double ans=0;
		foru(p,1,n){
			double mn=1e18;
			foru(q,1,m){
				chkmin(mn,dis(a[p],b[q]));
			}
			ans+=mn;
		}
...
signed main()
{
	...
	#ifndef CPEDITOR
	#ifndef ONLINE_JUDGE
	// if(freopen("destiny1.in","r",stdin));
	if(freopen("destiny.in","r",stdin));
	if(freopen("destiny.out","w",stdout));
	#endif
	#endif
