...
int inv[30000005];
int invpre[30000005];
int invvpre[30000005];

int M[30000005];

// constexpr auto gen_i_i(){
// 	array<int,20000005> ret{};
...
void pre_init(){
	...
	ifac[30000000]=qpow(fac[30000000],mod-2);
	ford(i,30000000-1,0){
		ifac[i]=mul(ifac[i+1],i+1);
		inv[i+1]=mul(ifac[i+1],fac[i]);
	}
	inv[0]=0;

	foru(i,1,30000000){
		invpre[i]=add(invpre[i-1],inv[i]);
		invvpre[i]=add(invvpre[i-1],mul(invpre[i],i));
	}
...
void solve(bool SPE){ 
	...
	foru(k,1,K){
		...
		int mi=mul(M[k+1],k+1);
		int mi_1=M[k];

		mdd(sum,mul(k,k-1,inv[2],fac[k],rmv(mi,mi_1),invpre[k]));
		mmv(sum,mul(invvpre[k-1],fac[k],rmv(mi,mi_1)));

		mdd(sum,mul(k,k-1,inv[2],fac[k],n-k,rmv(M[k+1],mul(M[k],inv[k]))));
		mdd(sum,mul(k,fac[k],n-k,M[k+1]));

		mdd(ans,mul(sum,inv[k]));
	}
...
signed main()
{
	...
	#ifndef CPEDITOR
	#ifndef ONLINE_JUDGE
	// if(freopen("castleintheair4.in","r",stdin));
	if(freopen("castleintheair.in","r",stdin));
	if(freopen("castleintheair.out","w",stdout));
	#endif
	#endif
