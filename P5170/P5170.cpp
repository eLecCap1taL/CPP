...
int n,a,b,c;
void solve(bool SPE){ 
	n=RIN,a=RIN,b=RIN,c=RIN;

	int ans1=(b/c)%mod;
	int ans2=mul(ans1,ans1);
	int ans3=0;

	auto res=Euclid::calc(a,b,c,n,Info(0,1,0,0,0),Info(1,0,0,0,0));
...
signed main()
{
	...
	#ifndef CPEDITOR
	// if(freopen("P51701.in","r",stdin));
	#ifdef ONLINE_JUDGE
	// if(freopen("P5170.in","r",stdin));
	// if(freopen("P5170.out","w",stdout));
	#endif
	#endif
