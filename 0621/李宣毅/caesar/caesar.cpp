...
int judge_caesar_rpre(int L,int R,int cR){
	if(L<1)	return 0;
	int ret=0;

	int clen=min(cR-R+1,L);

	int cL=L-clen+1;
	cR=R+clen-1;

	int cmx=BinarySearch(1,clen,[&](int len)->bool {
		return can_caesar(L-len+1,L,R,R+len-1);
	},0);
	...
	cL=L-cmx+1;
	cR=R+cmx-1;
	// if(cmx<clen)	return ret;

	int rmx=BinarySearch(1,min(cL-1,n-cR),[&](int len)->bool {
...
int judge_caesar_lsuf(int L,int R,int cL){
	if(R>n)	return 0;
	int ret=0;

	int clen=min(L-cL+1,n-R+1);
...
void solve(bool SPE){ 
	...
	// cout<<judge_caesar_rpre(2,3,4)<<endl;
	// return ;
	...
	foru(o,1,q){
		...
		foru(i,1,n){
			...
			if(l-1<=R && R<r){
				int res=judge_caesar_rpre(L-1,R+1,r);
				chkmax(ans,(R-L+1)+2*res);
			}
			if(l<L && L<=r+1){
				int res=judge_caesar_lsuf(L-1,R+1,l);
				chkmax(ans,(R-L+1)+2*res);
			}
		}
...
signed main()
{
	...
	#ifndef CPEDITOR
	#ifndef ONLINE_JUDGE
	// if(freopen("ex_caesar5.in","r",stdin));
