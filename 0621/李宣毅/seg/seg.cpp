...
public:
	int l,r,id;
	bool in(const Seg& x)const{
		if(x.l==l && r==x.r)	return id<x.id;
		return x.l<=l && r<=x.r;
	}
}a[5005];
...
void dfs(int u){
	...
	while(lst<r){
		auto it=st.lower_bound({lst,0});
		auto [L,R]=*it;

		chkmin(L,r);
		if(lst<L){
			auto itt=st.insert({lst,L}).fi;
			sum+=L-lst;
...
signed main()
{
	...
	#ifndef CPEDITOR
	#ifndef ONLINE_JUDGE
	// if(freopen("ex_seg1.in","r",stdin));
	if(freopen("seg.in","r",stdin));
	if(freopen("seg.out","w",stdout));
	#endif
	#endif
