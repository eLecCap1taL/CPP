...
public:
	...
	void calc(){
		...
		float r_score=1.0*r/n;
		float mxh_score=1.0*(8-mxh)/8;
		float cv_score=1.0*(81-cv)/81;
		float k_score=1.0*k/n;
		float fc_score=1.0/fc;

		score=5*r_score+5*mxh_score+100*cv_score+50*k_score;
	}
	State(const Space& x,const int& rd,const int& wk,int ID){
		d=x,r=rd,k=wk;
		id=ID;
		calc();
	}
