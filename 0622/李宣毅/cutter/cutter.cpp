...
void solve(bool SPE){ 
	...
	sort(a+1,a+1+n,[](const Forest& x,const Forest& y)->bool {
		return x.h<y.h;
	});
...
signed main()
{
	...
	#ifndef CPEDITOR
	#ifndef ONLINE_JUDGE
	// if(freopen("cutter1.in","r",stdin));
	if(freopen("cutter.in","r",stdin));
	if(freopen("cutter.out","w",stdout));
	#endif
	#endif
