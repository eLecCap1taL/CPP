...
vector<array<array<int,2>,2>> g[MAXN];
...
void calcg(int u,array<int,2> F){
	...
	foru(i,1,N){
		...
		if(u==1){
			mdd(g[u][i][1][1],mul(h[1],add(F[0],F[1])));
			mdd(g[u][i][0][1],mul(h[0],add(F[0],F[1])));
		}else{
			mdd(g[u][i][1][1],mul(h[1],add(F[0],F[1]),P));
			mdd(g[u][i][1][0],mul(h[1],F[0],Q));
			mdd(g[u][i][0][1],mul(h[0],add(F[0],F[1]),P));
			mdd(g[u][i][0][0],mul(h[0],F[0],Q));
		}

	}

	for(auto v:e[u]){
		calcg(v,{add(g[u][faid[v]][0][0],g[u][faid[v]][0][1]),add(g[u][faid[v]][1][0],g[u][faid[v]][1][1])});
	}
}
...
void solve(bool SPE){ 
	...
	foru(u,1,n){
		...
		while(1){
			...
			int up=g[fa[v]][faid[v]][0][1];

			mdd(ans,mul(L,M,up));
			
			mll(M,Q,ch1mul[fa[v]],f1inv[v]);
