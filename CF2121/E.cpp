...
int f[20][2][2];
int dp(int u,bool lm_bt,bool lm_tp){
	if(u==0)	return 0;
	if(f[u][lm_bt][lm_tp]!=-1)	return f[u][lm_bt][lm_tp];
	int l=0,r=9;
	if(lm_bt)	l=L[u];
	if(lm_tp)	r=R[u];
	int ret=INT_MAX;
	// cerr<<u<<' '<<lm_bt<<' '<<lm_tp<<' '<<l<<' '<<r<<endl;
	for(int i=l;i<=r;i++){
		// cerr<<' '<<lm_bt<<' '<<i<<' '<<L[u]<<endl;
		int res=dp(u-1,lm_bt&(i==L[u]),lm_tp&(i==R[u]));
		chkmin(ret,res+(i==L[u])+(i==R[u]));
	}
	return f[u][lm_bt][lm_tp]=ret;
}
