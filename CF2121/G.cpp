...
public:
	...
	Info operator + (const Info& x)const{
		return Info{num+x.num,si+x.si,sum+x.sum};
	}
};
class Node{
public:
...
void modify(int p,int pos,LL id,LL val){
	if(tr[p].l==tr[p].r){
		// cout<<tr[p].l-n<<" add "<<endl;
		tr[p].v.num++;
		tr[p].v.si+=id;
		tr[p].v.sum+=val;
		return ;
	...
	if(pos<=mid)	modify(lc(p),pos,id,val);
	else	modify(rc(p),pos,id,val);
	push_up(p);
}
Info query(int p,int l,int r){
	if(l>r)	return Info{0,0,0};
	if(l<=tr[p].l && tr[p].r<=r)	return tr[p].v;
	int mid=(tr[p].l+tr[p].r)>>1;
...
void solve(bool SPE){ 
	...
	ford(l,n,1){
		...
		modify(1,n+s[l],l,f[l]);
		// if(l<n)modify(1,n+s[l],l,f[l]);
		
		Info v=query(1,n+s[l-1],2*n);
		...
		// foru(r,l,n){
			...
			// }
		// }
		// cout<<l<<' '<<s[l]<<' '<<num<<' '<<v.num<<' '<<si<<' '<<v.si<<' '<<sum<<' '<<v.sum<<endl;
		num=v.num;
		si=v.si;
		sum=v.sum;
		
		
		ans+=2ll*sum-f[l-1]*num;
		ans+=(LL)(n-l+1-num)*(1-l+f[l-1]);
		ans+=(LL)(l+n)*(n-l+1)/2-si;
	}
