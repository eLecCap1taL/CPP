...
public:
	...
	Node(LL x=0,int p=0):mn(mkp(x,p)),mx(mkp(x,p)){}
	LL min()const{
		return mn.fi;
	}
	LL max()const{
...
bool check1(int i,int j){
	...
	Node p=pref[i];
	Node q=preg[j];
	if(p.min()<=q.min())	return check1(p.argmin()-1,j);
	if(q.max()>=p.max())	return check1(i,q.argmax()-1);
	return false;
}
bool check2(int i,int j){
	...
	Node p=suff[i];
	Node q=sufg[j];
	if(p.min()<=q.min())	return check2(p.argmin()+1,j);
	if(q.max()>=p.max())	return check2(i,q.argmax()+1);
	return false;
}
...
bool check(LL v){
	...
	foru(i,1,N){
		pref[i]=suff[i]=Node(P[i],i);
	}
	foru(i,1,M){
		preg[i]=sufg[i]=Node(Q[i],i);
	}
	...
	if(p.min()>q.min())	return false;
	if(q.max()<p.max())	return false;
	
	// cout<<p.argmin()<<' '<<q.argmax()<<endl;
// 	
	// foru(i,1,N){
		// foru(j,1,M){
			// cout<<(P[i]>Q[j]);
	...
	bool lf=check1(p.argmin()-1,q.argmax()-1);
	bool rf=check2(p.argmin()+1,q.argmax()+1);
...
signed main()
{
	...
	#ifndef CPEDITOR
	// if(freopen("light4.in","r",stdin));
	// if(freopen("light.out","w",stdout));
	#ifdef ONLINE_JUDGE
	if(freopen("light.in","r",stdin));
	if(freopen("light.out","w",stdout));
	#endif
	#endif
